...
* **What relationship, if any, is there between drought severity and water level depths in ground water wells in Colorado over the last twenty years?**
...
Rather than blindly choosing one of the above indicators as the basis of this study,  all three were evaluated, and the results from each compared. 
...
### **_Round 5_**
While most of this study has focused on the state of Colorado as a whole. A stretch goal of this capstone was to map out drought by counties to get a better understanding of the regional differences across the state. The below images illustrate the same information as the bar chart above, but break it out by county rather than showing the mean values for the state as a whole. In the future, it  might be interesting to conduct these same hypothesis tests for difference regions of the state, as drought clearly differs not only year by year, but also region by region.
...
With regards to the first question, the data has illustrated that there *has* been a significant change in the PDSI indicator over the time period of 2000 - 2016. The shift over this particular time period was one from more drought to less drought. The other two indicators did *not* show a meaningful change over this time period. 
...
As demonstrated by both the hypothesis tests and the correlation matrix, there *are* differences between each of the three drought indicators. The PDSI indicator demonstrated the highest correlation with other other two indices, and would be the recommended indicator to move forward with in a study where only one can be utilized. 
